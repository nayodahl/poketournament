...
class Slugger
{
     ...
     */
    public function slugIt(string $text): string
    {
        $text = self::unaccent($text);

        return self::cleanIt($text);
    }
     ...
     */
    private static function cleanIt(string $text, string $separator = '-'): string
    {
        ...
        $text = (string) preg_replace('/(\\w)\'(\\w)/', '${1}${2}', $text);
        ...
        $text = (string) preg_replace('/([A-Z]+)([A-Z][a-z])/', '\1_\2', $text);
        $text = (string) preg_replace('/([a-z\d])([A-Z])/', '\1_\2', $text);
